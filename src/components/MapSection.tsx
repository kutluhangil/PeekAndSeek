...
function MapContent({ onBack, apiKey }: MapSectionProps) {
  ...
  const [gameState] = useState(() => {
    ...
    if (
      urlLat &&
      urlLng &&
      !isNaN(parseFloat(urlLat)) &&
      !isNaN(parseFloat(urlLng))
    ) {
    ...
    if (hl) {
      if (
        savedState &&
        savedState.hiddenLocation.lat === hl.lat &&
        savedState.hiddenLocation.lng === hl.lng
      ) {
  ...
  const [hiddenLocation, setHiddenLocation] = useState(
    gameState.hiddenLocation,
  );
  const [guessesCount, setGuessesCount] = useState(gameState.guessesCount);
  const [previousDistance, setPreviousDistance] = useState<number | null>(
    gameState.previousDistance,
  );
  ...
  const winThreshold =
    difficulty === "hard" ? 0.05 : difficulty === "easy" ? 0.5 : 0.1;
  ...
  let clue = hideDirection
    ? "Directional sensors disabled due to protocol."
    : `Head ${direction}`;
  if (landmark) {
    clue += ` towards area of ${landmark}`;
  }
  clue += hideDirection
    ? ` Radius anomaly: ${distance.toFixed(1)} km.`
          ...
          >
            <div className="bg-card w-full max-w-md p-8 shadow-2xl border border-border">
              ...
              <p className="text-[13px] text-foreground/80 font-light leading-relaxed mb-8 text-center px-4">
                Pan the map to search for the hidden coordinate. Use the
                directional clues and street view ocular to narrow the radius.
                You have exactly <strong className="text-foreground">10</strong>{" "}
                guesses to pinpoint the location.
              </p>
              <button
              ...
              <div>
                <h2 className="text-[10px] uppercase tracking-[0.25em] text-muted mb-4 border-b border-border pb-2 flex justify-between items-center">
                  <span className="flex items-center gap-2">
                    <MapPin className="w-3 h-3" /> Fix
                  </span>
                  <div className="flex gap-4">
                    <span className="font-mono flex items-center shadow-inner gap-1">
                      <Clock className="w-3 h-3" />{" "}
                      {Math.floor(elapsedTime / 60)}:
                      {(elapsedTime % 60).toString().padStart(2, "0")}
                    </span>
                    <span className="font-mono">
                      {guessesCount} / 10 attempts
                    </span>
                  </div>
                </h2>
                    ...
                    {hideStreetView ? (
                      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
                        ...
                        <p className="text-[10px] text-muted font-mono">
                          Cartographic mode active. No remote visuals.
                        </p>
                      </div>
                    ) : streetviewError ? (
                      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
                      ...
                      <img
                        ...
                        alt="Streetview panorama limit"
                      />
                    ) : (
                      <div className="flex object-center justify-center h-full items-center">
                        <Target
                          className="w-6 h-6 text-muted animate-spin-slow opacity-50"
                          strokeWidth={1}
                        />
                      </div>
                    )}
            ...
            <div className="p-6 border-t border-border mt-auto bg-muted-bg/30 relative">
                ...
                {feedback && (
                  <motion.div
                    ...
                    className={`absolute -top-14 left-6 right-6 p-3 text-center text-[11px] uppercase tracking-wider font-bold border shadow-lg ${
                      feedback.toLowerCase().includes("copy")
                        ? "bg-blue-50 border-blue-200 text-blue-800"
                        : feedback.toLowerCase().includes("found")
                          ? "bg-green-50 border-green-200 text-green-800"
                          : feedback.toLowerCase().includes("game over") ||
                              feedback.toLowerCase().includes("too far")
                            ? "bg-[#f7f6f2] border-accent text-accent"
                            : feedback.toLowerCase().includes("warmer")
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  {isWaypointMode ? "Cancel Waypoint" : "Place Waypoint"}
                </button>
                <button
                  onClick={() => {
                    sounds.click();
                ...
                <button
                  onClick={() => {
                    ...
                    setRevealed(true);
                    setGameOver(true);
                    setFeedback("Signal Traced. Game Over.");
                  }}
                  disabled={revealed}
                ...
                <button
                  onClick={() => {
                    ...
                    url.searchParams.set(
                      "targetLat",
                      hiddenLocation.lat.toFixed(5),
                    );
                    url.searchParams.set(
                      "targetLng",
                      hiddenLocation.lng.toFixed(5),
                    );
      ...
      <div className="flex-1 relative h-full bg-background cartography-grid">
        ...
        <div className="absolute top-6 right-6 z-10 flex flex-col items-center gap-2">
          ...
          >
            ...
            >
              <div className="absolute text-accent text-[11px] font-bold top-1 left-1/2 -translate-x-1/2">
                N
              </div>
              <div
                className="w-[1.5px] h-[50%] bg-accent absolute bottom-1/2 left-1/2 -translate-x-1/2"
                style={{ clipPath: "polygon(50% 0%, 100% 100%, 0% 100%)" }}
              ></div>
              <div
                className="w-[1.5px] h-[50%] bg-muted absolute top-1/2 left-1/2 -translate-x-1/2"
          ...
          <div className="flex flex-col bg-card border border-border shadow-xl overflow-hidden rounded-md mt-2">
            <button
              onClick={() => map && map.setZoom((map.getZoom() || 14) + 1)}
              className="w-9 h-9 flex items-center justify-center hover:bg-muted-bg text-foreground border-b border-border transition-colors"
            >
              <Plus className="w-4 h-4" strokeWidth={1} />
            </button>
            <button
              onClick={() => map && map.setZoom((map.getZoom() || 14) - 1)}
              className="w-9 h-9 flex items-center justify-center hover:bg-muted-bg text-foreground transition-colors"
            >
        ...
        <GoogleMap
          ...
          styles={
            isDarkTheme ? cartographyNightMapStyles : cartographyMapStyles
          }
          options={{
            draggableCursor: isWaypointMode ? "crosshair" : undefined,
          }}
          onClick={(e) => {
        ...
        >
          <AdvancedMarker position={center} zIndex={50}>
            <motion.div
              animate={{ scale: isPulsing ? 1.4 : 1 }}
              transition={{ duration: 0.2 }}
              className="transition-transform duration-300 hover:scale-110 hover:drop-shadow-xl cursor-default"
            >
              <Pin
                background="#111111"
                borderColor="#111111"
          ...
          {waypoints.map((wp) => (
            ...
            >
              <div className="transition-transform duration-300 hover:scale-110 hover:drop-shadow-lg cursor-pointer group relative">
                ...
                >
                  <div
                    className="w-1.5 h-1.5 bg-amber-900 rounded-full"
                    style={{ margin: "auto" }}
                  />
                </Pin>
              </div>
      ...
      <div className="min-h-[100dvh] bg-background flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-card border border-border shadow-2xl p-8 text-center space-y-5">
          ...
          <h1 className="text-3xl font-serif italic text-foreground">
            Map session unavailable
          </h1>
          <p className="text-sm text-muted leading-relaxed">
            A Google Maps API key is required before the map view can
            initialize. Return to the landing screen and add a valid key to
            continue.
          </p>
          <button
